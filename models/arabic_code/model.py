...
class ArabicScorer(lang_model):
    """Adapter that makes a Hugging Face causal LM implement `lang_model`."""

    def __init__(self, params=None):
        params = params or {}
        model_name_param = params.get("hf_model_name", "aubmindlab/aragpt2-medium")
        ...
        print(f">>> [SCORER] Initializing Arabic Scorer with model: {model_name}")
    ...
    def _load_tokenizer(self, model_name):
        ...
        for kwargs in attempts:
            try:
                ...
                last_error = e
        raise RuntimeError(
            "Failed to load Arabic tokenizer '%s'. "
            "If this machine is offline, run `python download_model.py --arabic` "
    ...
    def _load_model(self, model_name):
        ...
        if self._is_local_dir(model_name) and not self._has_local_weights(model_name):
            raise RuntimeError(
                ...
                "Download with `python download_model.py --arabic`."
                % model_name
            )
